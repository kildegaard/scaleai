...
def calculate_gaps_area(squares, triangles, arrangement):
    ...
    bounding_rectangle_width = arrangement.count('square') * squares[0] + arrangement.count('triangle') * triangles[0] / 2
    bounding_rectangle_height = (arrangement.count('square') + arrangement.count('triangle')) * squares[0] * math.sqrt(3) / 2
    bounding_rectangle_area = bounding_rectangle_width * bounding_rectangle_height
...
def connect_shapes(squares, triangles):
    ...
    for i in range(len(squares)):
        for j in range(len(triangles)):
            ...
            for k in range(len(squares)):
                arrangement.append('square')
                if k < len(triangles):
                    arrangement.append('triangle')
    ...
    for arrangement in arrangements:
        ...
        gaps_area = calculate_gaps_area(squares, triangles, arrangement)
