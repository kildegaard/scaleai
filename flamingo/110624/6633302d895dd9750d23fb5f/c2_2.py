...
def calculate_gaps_area(arrangement, shape_size):
    # Calculate the total area of the shapes
    total_shapes_area = len(arrangement) * (shape_size**2)
    
    # Calculate the area of the bounding rectangle
    bounding_rectangle_width = shape_size * (len(arrangement) + 1) / 2
    bounding_rectangle_height = shape_size * math.sqrt(3) / 2
...
def connect_shapes(squares, triangles):
    ...
    for arrangement in arrangements:
        ...
        gaps_area = calculate_gaps_area(arrangement, 1)
