...
# Create a list of staff members for each role
...
def assign_shifts():
    for role, employees in staff.items():
        for day in range(7):
            ...
            # Assign half of the employees to the morning shift
            morning_shift = employees[:len(employees)//2]
            # Assign half of the remaining employees to the night shift
            night_shift = employees[len(employees)//2:][:len(employees)//4]
            # Assign the remaining employees to the afternoon shift
            afternoon_shift = employees[len(employees)//2:][len(employees)//4:]
