...
def calculate_evacuation_plan(num_players, player_strengths, requests):
    ...
    for i, (req_players, max_strength_limit) in enumerate(requests):
        # Iterate over each player in reverse order
        for j in range(num_players, req_players - 1, -1):
            # Calculate the sum of the strengths of the players that can be evacuated
            current_sum = sum(player_strengths[j - req_players : j])
            ...
            if current_sum <= max_strength_limit:
                ...
                max_strength[j] = max(
                    max_strength[j], max_strength[j - req_players] + current_sum
                )

                # If the current solution is better than the previous one, update the evacuation history
                if max_strength[j] > max_strength[j - 1]:
                    evacuation_history[i] = (
                        requests[i],
                        player_strengths[j - req_players : j],
                    )
