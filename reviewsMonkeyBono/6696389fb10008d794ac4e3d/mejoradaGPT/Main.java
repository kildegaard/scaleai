...
public class Main {
    ...
    private void placePieces() {
        List<Character> pieces = Arrays.asList('K', 'B', 'R', 'Q');
        for (char piece : pieces) {
            int col;
            do {
                col = random.nextInt(BOARD_SIZE);
            } while (board[0][col] != LIGHT_SQUARE && board[0][col] != DARK_SQUARE);
    ...
    private boolean isValidMove(int row, int col) {
        return row >= 0 && row < BOARD_SIZE && col >= 0 && col < BOARD_SIZE && board[row][col] != 'K'
                && board[row][col] != 'B' && board[row][col] != 'R' && board[row][col] != 'Q';
    }
