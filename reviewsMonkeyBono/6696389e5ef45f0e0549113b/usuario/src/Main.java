...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        waitForFutureCompletion(addTaskFuture);

        // Get all tasks
        Future<List<Task>> getAllTasksFuture = taskManager.getAllTasks();
        List<Task> tasks = waitForFutureResult(getAllTasksFuture);
        if (tasks != null) {
        ...
        waitForFutureCompletion(deleteTaskFuture);

        // Get all tasks after deletion
        getAllTasksFuture = taskManager.getAllTasks();
        tasks = waitForFutureResult(getAllTasksFuture);
        if (tasks != null) {
            tasks.forEach(task -> LOGGER.info("Retrieved task: " + task));
        }
    }
    ...
    private static void waitForFutureCompletion(Future<?> future) {
        try {
            future.get();
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
